...
checkuser=$(curl -s -i https://$usernametellonym.me -L | grep -o 'User Not Found')
...
curl --socks5-hostname localhost:9050 -i -s -L -H $"Host: "$username"tellonym.me" -H $'User-Agent: Mozilla/5.0 (X11; Linux x86_64; rv:52.0) Gecko/20100101 Firefox/52.0' -H $'Accept: */*' -H $'Accept-Language: en-US,en;q=0.5' -H $"Referer: https://"$username"tellonym.me/" $""$username"tellonym.me" > temp_user
...
IFS=$'\n'
send=$(curl --socks5 localhost:9050 -i -s -k  -X $'POST'     -H $"Host: "$username"tellonym.me" -H $'User-Agent: Mozilla/5.0 (X11; Linux x86_64; rv:52.0) Gecko/20100101 Firefox/52.0' -H $'Accept: */*' -H $'Accept-Language: en-US,en;q=0.5' -H $'Accept-Encoding: gzip, deflate' -H $"Referer: https://"$username"tellonym.me/" -H $'Content-Type: application/x-www-form-urlencoded; charset=UTF-8' -H $'X-Requested-With: XMLHttpRequest' -H $'Content-Length: 256' -H "Cookie: "$cookie"; mnet_session_depth=1%7C1534551935648; session_depth=testingpurposestellonym.me%3D1%7C355476528%3D1" -H $'Connection: close'     -b ""$cookie"; mnet_session_depth=1%7C1534551935648; session_depth=testingpurposestellonym.me%3D1%7C355476528%3D1"     --data-binary "__RequestVerificationToken="$token"&userId="$userid"&text="$message"&captchaResponse="     $"https://"$username"tellonym.me/Messages/SendMessage" | grep 'HTTP/1.1 200'; echo $?)
